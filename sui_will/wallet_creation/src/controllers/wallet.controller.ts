...
// Use predefined network configuration for Testnet
// const provider = new JsonRpcProvider(Network.TESTNET); // Use TESTNET for development
...
import express from 'express';
import { JsonRpcProvider, Ed25519Keypair, TransactionBlock } from '@mysten/sui';
import { encrypt, decrypt } from '../utils/crypto';
import { WalletModel } from '../models/wallet.model';
import * as crypto from 'crypto';
import { generateMnemonic } from 'bip39'; // Added for mnemonic generation

// Use predefined network configuration for Testnet
const provider = new JsonRpcProvider({ url: 'https://fullnode.testnet.sui.io' }); // Updated for clarity
...
export const createWallet = async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Generate mnemonic and keypair
    const mnemonic = generateMnemonic(); // Using bip39
    const keypair = Ed25519Keypair.deriveKeypair(mnemonic);
    const address = keypair.getPublicKey().toSuiAddress();
    const privateKeyBase64 = Buffer.from(keypair.getSecretKey()).toString('base64'); // Updated to getSecretKey
...
export const sendSui = async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Create and sign transaction
    const tx = new TransactionBlock();
    const [coin] = tx.splitCoins(tx.gas, [amount * 10 ** 9]); // Simplified amount handling
